...
account_type = st.radio("💵 ประเภทบัญชี", ["Standard", "Cent"])
...
if calculate:
    if end_price >= start_price:
        ...
        # --- สร้างตาราง: ไม้แรก = ขาดทุนมากที่สุด ไล่ลงไปไม้สุดท้าย ---
        for i in range(orders):
            price_current = start_price - step * i
            # loss = ไม้แรกขาดทุนมากที่สุด, ไล่ลงไม้สุดท้าย
            loss_current = (start_price - price_current + step) * contract_size * lot_size
            total_loss += loss_current
            df_data.append({
                "ไม้ที่": i + 1,
                "ราคาทอง ($)": round(price_current, 2),
                "ขาดทุน ($)": round(loss_current, 2)
            })
        ...
        # ✅ แก้ emoji ใน f-string ให้ถูกต้อง
        st.markdown(f"💰 ขาดทุนสะสมทั้งหมด ≈ {total_loss:,.2f} {'เซ็น' if account_type=='Cent' else 'USD'}")
